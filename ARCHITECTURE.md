...
**Adaptive contact behavior — the withhold predicate (`docs/A3-DESIGN.md §10A.3`).** A3-D's only mechanism for exercising the third `EVAL.md §3.4` advantage source (within-episode adaptive contact) is a single predicate, evaluated inline in `a3d_policy`: `withhold_applies = observations >= 2 and not any_engaged`, where `observations` counts the automatic failure email plus every contact sent so far in the episode. It gates exactly four of the sixteen rules (`R-09`, `R-10`, `R-13`, `R-15`) — most consequentially `R-13`, which sends a second contact on day 3 for the `card_expired`/`debit_instrument_blocked`/`card_not_enabled_group` bucket only if this predicate is false. **Day 9 Stage 2/3 diagnostic analysis of the sealed holdout ledger** (`docs/analysis/DAY9-DECOMPOSITION.md`) found this specific gate — not `STOP`, not the gate/executor layer, not a remedy mismatch — to be the dominant, structurally traceable mechanism behind A3-D's holdout invoice-recovery deficit against its bounded comparators (§7 below has the full figures). This is a diagnostic finding about the already-frozen, already-scored A3-D policy as it actually ran on holdout — it does not describe a different or modified policy, and the predicate itself is unchanged from what `docs/A3-DESIGN.md §10A.4` has specified since before any A3-D episode was ever executed.

**A3-LLM is a separate, development/tuning-only arm — not part of the shipped, holdout-scored A3-D policy.** It shares A3-D's runner, gate, executor, and ledger schema (`rrx.harness.runner.run_episode_a3`, same as A3-D), but its decisions come from a prompted `gpt-5-mini` call (`rrx.agent.planner.A3LLMPolicy`) rather than the fixed 16-rule table, and it exists in the codebase purely as a dev-tuned, ablation-comparison arm against A3-D (`EVAL.md §4.2`). **It is excluded from the holdout split entirely** (`EVAL.md §7.1` item A) — the sealed holdout result this document otherwise describes is A3-D's alone, and no figure anywhere in this repository attributes A3-LLM a holdout outcome of any kind.
