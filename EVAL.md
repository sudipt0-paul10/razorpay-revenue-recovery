...
| `stress` | 300 | 5,000–5,299 | Independent invariant check `[CORRECTION, eval-spec-v1.9]` |

All `[DESIGN]`. Every holdout run — including unsuccessful ones — is logged in `results/holdout_runs.md`.

**`stress` — normative definition `[CORRECTION, eval-spec-v1.9]`**

The `stress` split is 300 episodes at seeds 5,000–5,299, generated by the
frozen ordinary population distribution in `configs/population.yaml` —
the same distribution used for `dev` and `holdout`. The split changes the
RNG substream only, per §6; it applies no population override, no condition
filter, no value filter, and no reachability filter. There is no adversarial
population construction in this project.

Its purpose is to check that the §5.2 invariants hold on an independent,
never-tuned-against sample — not to test behaviour under an adversarial
distribution. `stress` is not a scored comparison, carries no single-use
restriction (unlike `holdout`, §3.5 above), and remains re-runnable.

**What this corrects.** Prior versions of this section described `stress`
as "Adversarial" and characterized it as comprising all-cancelled,
all-halted-at-open, high-value-only, and unreachable-customer cohorts.
None of those four constructions was ever operationalized. No code
implementing them exists in the repository or in its git history; no
allocation of the 300 episodes among the four categories was ever
specified; and "unreachable customer" was never given an operational
definition in terms of the six-field `LatentState`.
`src/rrx/harness/splits.py::stress_indices()` has returned
`range(5000, 5300)` — ordinary population, seed-defined — since it was
written. The prose described an intention; the repository implemented
the definition above. This correction removes the unimplemented prose
rather than leaving a specification that a reader could reasonably take
as a description of what was run.

**Consequence for the strength of the claim, stated plainly.** The
`stress` leg of criterion 1 establishes that the §5.2 invariants hold on
a third independent sample from the frozen population. It does **not**
establish invariant-holding under adversarial, shifted, or worst-case
populations, and no artifact produced by this project may describe it as
adversarial, as a robustness test in the distribution-shift sense, or as
evidence of worst-case behaviour. Robustness to population-parameter
variation is the separate concern of the §6A sensitivity sweep, and is
evidenced only to the extent that sweep was actually executed.

**Criterion 1 is unchanged.** The §5.2 invariants must still hold on
`dev`, `holdout` **and** `stress`. This correction changes what the word
`stress` denotes, not what is required of it.
...
with the `dev` row above.

[CORRECTION, eval-spec-v1.9] The sentence originally here — "`holdout`/
`stress` are not yet exercised by any code in this repository" — recorded
the true state at `eval-spec-v1.4` (2026-08-27) and is preserved above
unrewritten as of that entry's own authorship. It is now stale on one
half: `stress` was exercised for the first time in Stage 7.3
(`9ab5440`, 2026-08-29), against exactly the definition corrected above —
`stress_indices()`'s ordinary-population seed range was not changed to
produce that run; this correction only brings the specification's prose
into agreement with what `stress_indices()` has always returned.
**`holdout` remains unexercised** — no `holdout_indices(authorized=True)`
call has occurred anywhere in this repository's history, before or after
this correction.
...
8. **[ADDED, eval-spec-v1.4] The 5% cancelled-at-open bucket produces
   ...
   §3.2/§5 "restraint" observed on this bucket is **enforced by the
   environment, not demonstrated by the agent** — no pitch, README, or
   results claim may describe it as evidence of A3's restraint or
   judgement. Flagged for definitive verification once the A3 runner is
   implemented (`docs/A3-DESIGN.md §7, §20, §21`).

   > `[CORRECTION, eval-spec-v1.9]` This item's reference to "the
   > all-`cancelled` `stress` cohort" is stale. No all-cancelled stress
   > cohort was ever implemented; see §3.5. The `stress` split draws from
   > the ordinary population, in which cancelled-at-open episodes occur at
   > their population rate rather than universally. The substantive point
   > of this item is unaffected: for cancelled-at-open episodes, where they
   > occur, the absence of contacts is enforced by the environment —
   > episodes terminate at T=0 before any tick runs — and is not evidence of
   > the agent's restraint or judgement.
